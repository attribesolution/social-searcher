...
DailymotionAPI.GRANT_TYPES = _.reduce(
  GRANT_TYPES,
  function(acc, g) {
    acc[g.toUpperCase()] = g;
    return acc;
...
DailymotionAPI.prototype.setCredentials = function(grantType, credentials) {
  if (GRANT_TYPES.indexOf(grantType) === -1) {
    throw new Error(
      "DM.API :: Given grant_type in setCredentials() does not match any known type",
    );
  }
 ...
 */
DailymotionAPI.prototype.createToken = function(next, myArg) {
 ...
 * @param  {Function} next Callback for when the token is indeed refreshed. Can be called with 1 param if error
 */
DailymotionAPI.prototype.refreshToken = function(next, myArg) {
...
DailymotionAPI.prototype.api = function(verb, endpoint, data, callback) {
  ...
  if (!!this._expirationTimestamp && this._expirationTimestamp <= Date.now()) {
    if (this._authFailed) {
      return callback(
        new Error(
          "DM.API :: Authentication failed twice, check your credentials",
        ),
        null,
        {},
      );
  ...
  return request(opts, function(e, r, body) {
    if (typeof callback === "function") {
      try {
        ...
        return callback(jsonException, r, {});
      }
      if (!!res.error) {
...
DailymotionAPI.prototype.get = function(endpoint, data, callback) {
  this.api("GET", endpoint, data, callback);
};
DailymotionAPI.prototype.post = function(endpoint, data, callback) {
  this.api("POST", endpoint, data, callback);
};
DailymotionAPI.prototype.put = function(endpoint, data, callback) {
  this.api("PUT", endpoint, data, callback);
};
DailymotionAPI.prototype.patch = function(endpoint, data, callback) {
  this.api("PATCH", endpoint, data, callback);
};
DailymotionAPI.prototype.del = function(endpoint, data, callback) {
  this.api("DELETE", endpoint, data, callback);
};
DailymotionAPI.prototype.head = function(endpoint, callback) {
  this.api("HEAD", endpoint, callback);
};
DailymotionAPI.prototype.options = function(endpoint, callback) {
  this.api("OPTIONS", endpoint, callback);
};
...
DailymotionAPI.prototype.upload = function(options) {
  if (!options.filepath || !options.meta) {
    return (
      !!options.done &&
      options.done(
        new Error("DM.API :: Filepath or meta not given in upload method"),
        null,
      )
    );
  }
  ...
  if (!fs.existsSync(options.filepath)) {
    return (
      !!options.done &&
      options.done(new Error("DM.API :: Filepath not found"), null)
    );
  }
            ...
            request(
              ...
              function(e, r, body) {
                ...
                try {
                  ...
                  options.progress(jsonException, r, {});
                } finally {
        ...
        function(e, r, body) {
          ...
          try {
            ...
            if (typeof options.done === "function")
              options.done(jsonException, null);
          } finally {
